...
if __name__ == "__main__":
    ...
    for idname in id2obj.keys():

        temp_list = id2obj[idname]
        tt = id2kpbb[idname]
        ...
        for temp in temp_list:
            ...
            tt['keypoints'].append(temp['keypoints'])
            tt['boxes'].append([x1, y1, x2, y2])
            tt['scores'].append(temp['score'])
            tt['confidences'].append(confidence)

        width, height = Image.open(f'./set/{idname}').size
